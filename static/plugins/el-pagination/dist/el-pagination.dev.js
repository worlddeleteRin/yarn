...
(function ($) {
  $.fn.endlessPaginate = function (options) {
    ...
    var getContext = function getContext(link) {
      return {
    ...
    return this.each(function () {
      var element = $(this),
          loadedPages = 1; // Twitter-style pagination.
      ...
      element.on('click', settings.moreSelector, function () {
        var link = $(this),
            ...
            loading = container.find(settings.loadingSelector); // Avoid multiple Ajax calls.
        ...
        link.hide();
        loading.show();
        var context = getContext(link); // Fire onClick callback.

        if (settings.onClick.apply(html_link, [context]) !== false) {
          var data = 'querystring_key=' + context.key; // Send the Ajax request.
      ...
      element.on('click', settings.pagesSelector, function () {
        var link = $(this),
            html_link = link.get(0),
            context = getContext(link); // Fire onClick callback.

        if (settings.onClick.apply(html_link, [context]) !== false) {
          var page_template = link.closest(settings.pageSelector),
              data = 'querystring_key=' + context.key; // Send the Ajax request.
